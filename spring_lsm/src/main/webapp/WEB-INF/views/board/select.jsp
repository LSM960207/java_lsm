	...
	<div class="container">
		...
		<div class="form-group">
			...
			<div class="form-group mt-5">
				...
				<button class="btn btn-outline-success col-12 btn-comment-insert">댓글 등록</button>
			</div>
	...
	let bd_num = '${board.bd_num}';
		...
		$(function(){
			...
			$('.btn-comment-insert').click(function(){
				...
				if('${user.me_id}' == ''){
					if(confirm('로그인한 회원만 댓글 작성이 가능합니다. 로그인을 하시겠습니까?')){
						location.href = '<%=request.getContextPath()%>/login'
						return;
					}
				}
		...
		$(function(){
		 ...
		 $(document).on('click', '.btn-comment-update-cancel', function(){
			...
			$(document).on('click', '.btn-comment-reply', function(){
				let id = '${user.me_id}';
				if(id == ''){
					if(confirm('댓글에 답글을 달기 위해선 로그인을 해야합니다. 로그인하시겠습니까?')){
						location.href = '<%=request.getContextPath()%>/login'
						return;
					}
				}
				//답글을 누른 댓글에만 답글을 입력하는 창이 나오게 하기 위해 모든 답글취소 버튼을 클릭해서 없앰
				$('.btn-cancel-reply').click();
				$('.btn-comment-update-cancel').click();
