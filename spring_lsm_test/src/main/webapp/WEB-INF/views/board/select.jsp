	...
	<div class="container">
		<c:if test="${board.bd_del == 'N' }">
			...
			<div class="list-comment">
			    ...
			    <button class="btn btn-outline-success" style="display: block">답글</button>
			    <div class="btn-box">
			    	<button class="btn btn-outline-success" style="display: block">수정</button>
			    	<button class="btn btn-outline-danger  mt-1" style="display: block">삭제</button>
			    </div>
			  </div>
			</div>
	...
	<script type="text/javascript">
		...
		$(function(){
			...
			$(document).on('click', '.btn-co-reply', function(){
				...
				str += 			'<button class="btn btn-outline-success btn-reply-complete" style="display: block">등록</button>';
				str += 			'<button class="btn btn-outline-danger mt-1 btn-reply-cancel" style="display: block">취소</button>';
				str += 		'</div>';
				str += '</div>';
				$(this).parent().siblings('.media-body').append(str);
				$(this).parent().hide();	//답글, 수정, 삭제버튼 감추기
			})
			...
			$(document).on('click', '.btn-reply-cancel', function(){
				$(this).parents('.comment-body').find('.btn-box').show();	//답글, 수정, 삭제 버튼 보여주기
				$(this).parents('.box-reply').remove(); //답글창, 등록, 취소 버튼을 삭제해주기
			})
			getCommentList(cri);
		})
		...
		function commentListSuccess(data){
			...
			$('.btn-co-update').click(function(){
				...
				str +=	 	'<button data-target="'+co_num+'" class="btn btn-outline-success btn-co-complete" style="display: block">등록</button>';
