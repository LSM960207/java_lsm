...
public class Ex6_Program_Student {
	 ...
	 * 1. 기능을 정리 => 주석으로
	 * 	- 학생 정보 입력
	 * 		- 학년, 반, 번호, 이름, 국어, 영어, 수학 성적을 한 번에 입력받음
	 * 		- 입력한 학생 정보를 저장
	 * 	- 학생 정보 출력
	...
	public static void main(String[] args) {
		...
		for( ; menu!=5; ) {
			...
			case 1:
				...
				// Ex6_Student의 생성자를 이용하여 객체를 생성한 후 학생 배열 std에서 index 번지에 저장
			...
			case 3:
				// 학년, 반, 번호를 입력받음
				System.out.println("수정할 학생 정보를 입력하세요(학년, 반, 번호) : ");
				grade = scan.nextInt();
				classNum = scan.nextInt();
				num = scan.nextInt();
				...
				for(int i = 0; i < index; i++) {
					...
					// 이름, 국어, 영어, 수학 성적을 입력 받은 후
						System.out.println("수정할 학생 이름과 성적을 입력하세요(이름, 국어, 영어, 수학순) : ");
						name = scan.next();
						kor = scan.nextDouble();
						eng = scan.nextDouble();
						math = scan.nextDouble();
			...
			case 4:
				...
				// delIndex가 0 이상이면
				if(delIndex >= 0) {
				//반복문을 이용하여 delIndex번지부터 index-2번지까지 다음 번지에 있는 정보를 현재 번지에 저장
