...
public class Ex1_Method1_Baseball {
	...
	public static void main(String[] args) {
		 ...
		 */
			...
			// 저장된 개수가 3개가 될 때까지 : count가 3이 아니면 동작(count가 3보다 작으면 동작)
			for(int count = 0; count < 3;) {
				// 1~4 사이의 랜덤한 수 생성
				int r = (int)(Math.random() * (max - min +1) + min);
				
			// 배열에 count개만큼 랜덤한 수와 비교하여 있는지 없는지 확인해서 없으면 배열 count 번지에 랜덤한 수 저장하고 count를 1 증가
	 ...
	 * 1S1B, O, 1S,2B
	 * 매개변수 : 스트라이크와 볼의 개수 => 정수 => int strike, int ball
	 * 리턴타입 : 없음 => void
	 * 메소드명 : printResult
	 * */
