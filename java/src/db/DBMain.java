...
public class DBMain {
	...
	public static void main(String[] args) {
		...
		*/
		...
		String st_name = scan.next();
		if(dbDao.upadteStudentName(st_num, st_name) == 1) {
	...
	public static Connection connect() {
		...
		try {
			...
			//Class.forName(driverName); 에서 예외가 발생한 것
			System.out.println(driverName + "클래스를 찾을 수 없습니다.");
		}catch(SQLException e) {
			//con = DriverManager.getConnection(url, user, pw); 에서 예외가 발생한 것
