...
public class Ex7_Mart {
	...
	/* 음료수 또는 박스과자를 관리하는 마트(종류는 최대 30개 => 음료수+박스 합쳐서 30)
	 * 1. 제품 등록(신규제품)
	 * 2. 제품 입고
	 * 3. 제품 선택
	 * 4. 제품 구매
	...
	public static void main(String[] args) {
		...
		do {
			...
			case 4:
				...
				if(sum > buyPrice) {
					//결제를 취소할건지 물어봄
					System.out.println("결제를 취소하겠습니까?(취소 시 장바구니는 비워집니다. Y / N) : ");
	...
	/* 기능 : 메뉴를 출력하고, Scanner를 통해 메뉴를 입력받아 입력받은 메뉴를 알려주는 메소드
	...
	/* 기능 : 제품등록에 대한 서브 메뉴를 출력하고, Scanner를 통해 서브 메뉴를 입력받아 서브 메뉴를 알려주는 메소드
	 * 매개변수 : Scanner => Scanner scan
	 * 리턴타입 : 입력받은 메뉴 => 정수 => int
	 * 메소드명 : selectSubMenu
	 * */
	...
	/* 기능 : 선택한 subMenu에 맞는 제품 정보를 Scanner를 통해 입력받아 제품 객체를 생성한 후
	 * 		 제품을 알려주는 메소드
	 ...
	 * 		제품 리스트에서 해당 제품을 수량에 맞게 꺼내서 제품을 돌려주는 메소드
	 * 매개변수 : 스캐너, 제품리스트(마트가 관리하는) => Scanner scan, Product list[],int listCount 
	...
	public static void returnProductList(Product list[], int listCount, Product basket[], int basketCount) {
		...
		//판매 제품 목록과 장바구니 목록을 비교하여 제자리에 가져다 놓음
		for(int i = 0; i < listCount; i++) {
			for(int j = 0; j < basketCount; j++) {
				Product pi = list[i];	//판매하는 제품
				Product pj = basket[j];	//장바구니 제품
				//판매하는 제품 이름과 장바구니 제품 이름이 같으면
				if(pi.getName().equals(pj.getName())) {
					//pi.amount : 재고량, pj.amount : 구매하려고 선택했던 수량
					//장바구니에 담았던 제품을 제자리에 돌려놓음 => 판매하는 제품 수량에 장바구니 제품의 수량을 추가
