...
public class Ex17_Exception_Test3 {
	...
	public static void main(String[] args) {
		/*
		 * 중복되지않은 1 ~ 9 사이의 세 정수를 랜덤으로 생성하여 배열에 저장하고 출력하는 코드를 작성하세요. 단, 위의 내용을 메소드를 만들어서
		 * 활용 예외 처리를 해야함
		 */
		int arr[];
		int min = 1, max = 9;
	...
	/* 최소값과 최대값과 만들어야할 개수가 주어지면 중복되지않은 랜덤한 ㄱ밧이 들어있는 배열을 만들어서 반환하는 메소드*/
	public static int [] createRandomArray(int min, int max, int n) throws Exception {
		...
		if(max - min + 1 < n) {
			throw new Exception(min + "과 " + max + " 사이에서 " + n + "개의 중복되지않은 배열을 만들 수 없습니다.");
		}
		if(n <= 0) {
			throw new Exception(n +"개의 배열을 만들 수 없습니다.");
		}
		int arr[] = new int[n];
		int count = 0;
		while(count < n) {
