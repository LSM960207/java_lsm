...
-- 2020001	이순신	1995-05-01	청주시	010-1254-5678	마케팅부	대리	1년차	2020-03-02
-- 2022001	고길동	1998-12-31	청주시	010-3234-5678	영업부	사원	0년차	2022-03-02
insert into employee (em_num, em_name, em_birth, em_addr, em_phone, em_de_name, em_rn_name, em_year, em_enter) values
(2000001, '홍길동', '1980-01-01', '청주시', '010-1234-5678', '개발부', '차장', 1, '2000-03-02'),
(2000002, '임꺽정', '1980-01-01', '청주시', '010-1234-5679', '개발부', '차장', 2, '2000-03-02'),
...
insert into pay (pa_em_num, pa_date, pa_salary, pa_name, pa_year)
	select em_num, '2022-01-05', rn_base + rn_add * em_year, em_de_name, em_year
		from employee
		join `rank` on em_rn_name = rn_name
		where em_enter < '2022-01-05';
-- 2022년 2월 5일에 등록된 모든 사원들의 월급을 지급했다 (2월 월급을 지급)  
insert into pay (pa_em_num, pa_date, pa_salary, pa_name, pa_year)
	select em_num, '2022-02-05', rn_base + rn_add * em_year, em_de_name, em_year
		from employee
		join `rank` on em_rn_name = rn_name
		where em_enter < '2022-02-05';
-- 2022년 3월 5일에 등록된 모든 사원들의 월급을 지급했다 (3월 월급을 지급)  
insert into pay (pa_em_num, pa_date, pa_salary, pa_name, pa_year)
	select em_num, '2022-03-05', rn_base + rn_add * em_year, em_de_name, em_year
		from employee
		join `rank` on em_rn_name = rn_name
		where em_enter < '2022-03-05';
-- 2022년 4월 5일에 등록된 모든 사원들의 월급을 지급했다 (4월 월급을 지급)  
insert into pay (pa_em_num, pa_date, pa_salary, pa_name, pa_year)
	select em_num, '2022-04-05', rn_base + rn_add * em_year, em_de_name, em_year
		from employee
		join `rank` on em_rn_name = rn_name
