-- 6월 14일 CGV청주성안길에서 11:30에 상영하는 영화 <브로커>에 예매 가능한 좌석을 조회(예매된 좌석도 조회)
...
-- 현재 상영중인 영화 중 개봉일을 기준으로 최신영화 10개를 조회
...
select mo_title as 영화제목, sum(bo_amount) as 관람객수 from movie
	join (select * from screen where sc_start_time <= now()) as sc
		on sc_mo_num = mo_num
