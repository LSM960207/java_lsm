-- abc123 회원의 누적 구매금액을 조회
select bu_us_id, sum(pr_price) as 누적구매금액 from buy 
	join `option` on op_num = bu_op_num
	join product on pr_code = op_pr_code
    where bu_us_id = 'abc123';
    
select bu_us_id, sum(pr_price) as 누적구매금액
...
select op_pr_code as 제품고드, op_size as 옵션, count(bu_num) from `option`
...
select bu_us_id as 회원이름, pr_title as `구매한 제품 목록` from product
	join `option` on op_pr_code = pr_code
    join buy on bu_op_num = op_num
    where bu_us_id = 'abc123'
    group by pr_code;
