...
-- 지역이 경남이거나 충남이거나 경북인 사람을 조회
select userName from usertbl where addr in('경남', '충남', '경북');
select userName from usertbl where addr = '경남' or addr = '충남' or addr = '경북';
...
-- KHD 회원이 구매한 제품과 동일한 제품을 구매한 회원ID만 조회
select userID from buytbl where userID != 'KHD' and prodName = any(select prodName from buytbl where userID = 'KHD');
select userID from buytbl where userID != 'KHD' and prodName in(select prodName from buytbl where userID = 'KHD');
