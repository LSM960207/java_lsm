-- 2. 기존에 했던 추천/비추천을 변경해서 새로 추천/비추천을 하는 경우 업데이트하는 트리거
-- likes 테이블에 update를 하는 경우
-- => board 테이블에서 변경되기 전 추천/비추천의 수를 1 감소하고, 변경된 후 추천/비추천의 수를 1 증가
...
-- 위 update를 실행 후, 댓글 1번에 해당하는 board 테이블에 1번 게시글에 추천/비추천 수를 확인