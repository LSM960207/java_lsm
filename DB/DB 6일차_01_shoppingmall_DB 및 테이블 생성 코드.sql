...
-- 관리자 정보를 등록하는 쿼리를 작성 주소는 NULL 생일도  NULL
...
-- id가 abc123, 비밀번호가 abc123, 주소가 청주시, 생일이 2000-01-01인 회원이 회원가입을 했다
alter table user 
...
-- abc123 회원이 ABC001 제품 xl를 1개 구매했을 때 필요한 쿼리문을 작성
-- 구매시 입력한 주소는 청주시
...
-- abc123 회원이 DEF001 제품을 220과 260을 구매했다
-- 구매시 주소는 청주시
insert into buy(bu_us_id, bu_op_num, bu_address)
	select 'abc123', op_num, '청주시' from `option`
...
-- 크기 : l, 수량 : 10
insert into product(pr_code, pr_type, pr_title, pr_detail, pr_price)
	values('ABC002', '상의', '니트', '니트입니다.', 30000);
insert into `option`(op_size, op_amount, op_pr_code) values
('l', 10, 'ABC002');