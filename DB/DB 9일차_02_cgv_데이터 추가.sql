...
insert into seat(se_ci_num, se_name, se_type, se_use)
    ...
    (6, 'B1', '일반', 'Y'),(6, 'B1', '일반', 'Y'),(6, 'B3', '일반', 'Y'),
    (6, 'C1', '일반', 'Y'),(6, 'C2', '일반', 'Y');
insert into seat(se_ci_num, se_name, se_type, se_use)
	values(7, 'A1', '일반', 'Y'),(7, 'A2', '일반', 'Y'),(7, 'A3', '일반', 'Y'),
    (7, 'B1', '일반', 'Y'),(7, 'B1', '일반', 'Y'),(7, 'B3', '일반', 'Y'),
    (7, 'C1', '일반', 'Y'),(7, 'C2', '일반', 'Y');
insert into seat(se_ci_num, se_name, se_type, se_use)
	values(8, 'A1', '일반', 'Y'),(8, 'A2', '일반', 'Y'),(8, 'A3', '일반', 'Y'),
    (8, 'B1', '일반', 'Y'),(8, 'B1', '일반', 'Y'),(8, 'B3', '일반', 'Y'),
    (8, 'C1', '일반', 'Y'),(8, 'C2', '일반', 'Y');
    
-- CGV청주성안길의 브로커 6월 14일 화요일
...
-- abc123회원이 가입, 비밀번호는 abc123
insert into user(us_id, us_pw) values('abc123', 'abc123');
SELECT * FROM user;
-- abc123회원이 6월 14일 CGV청주성안길에서 11:30에 하는 영화 브코러 A1, A2 좌석을 예매. 가격은 자리당 10000원
-- 예매를 추가  book 테이블 데이터 추가
...
select * from book;
-- 예매 내역을 추가  bookdetail 테이블 데이터 추가
...
-- abc123회원이 6월 14일 CGV청주성안길에서 11:30에 하는 영화 브코러 C1, C2 좌석을 예매. 가격은 자리당 10000원
-- 예매를 추가  book 테이블 데이터 추가
insert into book(bo_us_id, bo_date, bo_state, bo_amount, bo_sc_num, bo_total_price)
	select 'abc123', now(), 'Y', 2, sc_num, 20000 from screen
		join movie on sc_mo_num = mo_num
