...
-- 공지사항은 최고 관리자만 작성할 수 있고, 회원 이상만 읽을 수 있다
-- 일반은 회원 이상 작성할 수 있고, 비회원도 읽을 수 있다
-- 문의사항은 회원 이상 작성할 수 있고, 회원 이상만 읽을 수 있다
insert into category(ca_name, ca_r_authority, ca_w_authority, ca_secret_pos)
	values('공지사항', 1, 10, 0), ('일반', 0, 1, 0), ('문의사항', 1, 1, 1);

-- abcd1234 회원이 일반 게시글에 제목 : 안녕하세요, 내용 : 만나서 반갑습니다. 라는 글을 등록했다
-- 최고관리자가 공지사항에 제목 : 공지사항1, 내용 : 공지사항입니다 라는 글을 등록했다
-- qwer1234회원이 문의 게시글에 제목 : 문의합니다, 내용 : 비번을 바꾸고 싶어요 라는 글을 비밀글로 등록했다
...
insert into board(bd_ca_name, bd_title, bd_contents, bd_us_id, bd_secret, bd_ori_num)
	select '일반', '안녕하세요1', '만나서 반갑습니다1.', 'abcd1234', 0, ifnull(max(bd_num),0)+1
		from board;
-- admin123 관리자가 3번 게시글에 답글을 남김. 제목 : 문의하신 내용에 답변입니다. 내용 : 비번찾기를 이용하세요.(5번 게시글)
-- (bd_depth=>2)
...
-- qwer1234 회원이 5번 게시글에 답글을 남김. 제목 : 그래도 안돼요. 내용 : 비번찾기를 이용해도 안됩니다.
insert into board(bd_ca_name, bd_title, bd_contents, bd_us_id, bd_secret, bd_ori_num, bd_depth)
	select '문의사항', '그래도 안돼요.', '비번찾기를 이용해도 안됩니다.', 'qwer1234', 0, 
		bd_ori_num, bd_depth+1 from board
		where bd_num = 5;
        
-- 3번 게시글을 조회했을 때 (게시글 내용을 확인했을 때) 필요한 쿼리 => 조회수 증가
update board set views = views + 1 where bd_num = 3;

-- 추천/비추천을 했다 => 해당 게시글/댓글에 사용자가 처음 추천/비추천하는 경우 => 등록, 아닌 경우 => 수정(삭제)

-- qwer1234 회원이 1번 게시글에 추천을 눌렀을 때 쿼리문 (게시글을 확인하고, 추천버튼을 클릭했다고 가정)
insert into likes(li_us_id, li_state, li_target, li_target_num)
	values('qwer1234', 1, 'board', 1);
    
-- qwer1234 회원이 1번 게시글에 추천을 한 번 더 눌러 추천을 취소했을 때 쿼리문
update likes set li_state = 0 
	where li_us_id = 'qwer1234' and li_target_num = 1 and li_target = 'board';
    
-- 일반, abcd1234, 제목입니다2, 내용입니다
-- 첨부파일을 2개 추가해서 게시글을 등록. 첨부파일을 추가할 때 추가해야하는 게시글의 번호를 안다고 가정
