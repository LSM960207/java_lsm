...
<style>
.btn-del {
	padding: 0; border: none; background-color: transparent; color: #dc3545;
}
    	...
    	<c:forEach items="${list}" var="bo" varStatus="status">
      	<tr>
        	<td>${pm.totalCount - (pm.cri.page-1) * pm.cri.perPageNum - status.index}</td>
					<td>
						<a href="<c:url value="/board/select?bd_num=${bd.bd_num}"></c:url>">${bo.bd_title}</a>
					</td>
					<td>${bo.bd_reg_date_str}</td>
					<td>
						<a class="btn btn-outline-warning" href="<c:url value="/admin/notice/update?bd_num=${bo.bd_num}"></c:url>">수정</a>
